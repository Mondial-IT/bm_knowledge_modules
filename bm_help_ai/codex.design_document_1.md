...
The goal of this module is **not** to replace existing Drupal help systems, but to **aggregate, orchestrate, and enhance** them—optionally using AI—while preserving existing contracts, editorial intent, and fallback behavior. Aggregation and metadata now flow through the shared `bm_knowledge_ai` layer (adapters + classification + context); keep bm_help_ai focused on the Help adapter and UI.
